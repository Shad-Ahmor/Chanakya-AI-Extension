...
class MemoryRetriever {
     ...
     */
    async retrieve(taskDescription, topK = 5) {
        try {
    ...
    calculateHybridScore(memory, taskDescription) {
    ...
    formatMemoriesForPrompt(memories) {
        ...
        for (let i = 0; i < memories.length; i++) {
            ...
            if (m.task)
                prompt += `- **Context**: ${m.task}\n`;
            if (m.memory_type === 'mistake') {
                if (m.error)
                    prompt += `- **Error/Failure**: ${m.error}\n`;
                if (m.root_cause)
                    prompt += `- **Root Cause**: ${m.root_cause}\n`;
                if (m.prevention)
                    prompt += `- **PREVENTION RULE**: ${m.prevention}\n`;
            }
            else if (m.memory_type === 'procedural' || m.memory_type === 'semantic') {
                if (m.general_lesson)
                    prompt += `- **Lesson/Procedure**: ${m.general_lesson}\n`;
            }
            else {
                if (m.action)
                    prompt += `- **Action Taken**: ${m.action}\n`;
                if (m.result)
                    prompt += `- **Result**: ${m.result}\n`;
            }
            prompt += '\n';
        }
...
//# sourceMappingURL=MemoryRetriever.js.map