...
import { LLMEngine, StreamCallbacks } from './llmEngine';
import { SecurityGuardrails } from '../utils/securityGuardrails';
import { EvaluationService } from './evaluationService';
import { CompressionAgent } from './compressionAgent';
...
export class LLMGateway {
   ...
   */
  private determinePrimaryModel(allModels: readonly ModelConfig[], requestedModel: ModelConfig, optimizerConfig: any): ModelConfig {
    if (!optimizerConfig || !optimizerConfig.taskType) return requestedModel;
    ...
    if (optimizerConfig.taskType === 'chat' || optimizerConfig.taskType === 'explanation') {
      const fastModel = allModels.find(m => 
        (m.id || m.name || '').toLowerCase().includes('haiku') || 
    ...
    if (optimizerConfig.taskType === 'coding' || optimizerConfig.taskType === 'refactoring') {
       const smartModel = allModels.find(m => 
        (m.id || m.name || '').toLowerCase().includes('sonnet') || 
  ...
  public async streamChat(params: {
    ...
    const semanticCache = SemanticCache.getInstance();
    ...
    const sanitizedPrompt = SecurityGuardrails.redactSensitiveInfo(prompt);
    ...
    if (existingMessages && existingMessages.length > 5) {
      ...
      if (roughTokens > 4000) {
        ...
        try {
          const compressedText = await CompressionAgent.getInstance().compressContext(toCompress);
          optimizedMessages = [
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        const interceptedCallbacks: StreamCallbacks = {
          onChunk: callbacks.onChunk,
          onError: callbacks.onError,
          ...(callbacks.onOptimizationStats ? { onOptimizationStats: callbacks.onOptimizationStats } : {}),
          onComplete: (fullText) => {
            const validation = SecurityGuardrails.validateGeneratedCode(fullText);
            ...
            EvaluationService.getInstance().evaluateResponse(sanitizedPrompt, fullText, latencyMs, tokenCount)
