...
import { McpToolDefinition, McpResourceDefinition, McpPromptDefinition } from '../types/mcp';
import { GraphifyService } from './graphifyService';

export class EmbeddedMcpServer {
  private static instance: EmbeddedMcpServer;
  private memoryStore: Map<string, any> = new Map();

  private constructor() {}

  public static getInstance(): EmbeddedMcpServer {
  ...
  public async executeTool(toolName: string, args: Record<string, any>): Promise<string> {
    ...
    switch (toolName) {
      ...
      case 'chanakya_workspace_search': {
        ...
        try {
          const out = execSync(`git grep -n -I -i -E "${query.replace(/"/g, '\\"')}"`, {
            cwd: rootPath,
            encoding: 'utf-8',
            timeout: 4000
          });
      ...
      case 'chanakya_diagnostics': {
        ...
        for (const [uri, fileDiags] of diags) {
          for (const d of fileDiags) {
            ...
            if (args.severity && args.severity !== 'all' && sev !== args.severity) continue;
            results.push({
      ...
      case 'chanakya_git_diff': {
        try {
          const status = execSync('git status --short', { cwd: rootPath, encoding: 'utf-8', timeout: 3000 });
          const diff = execSync(args.stagedOnly ? 'git diff --cached' : 'git diff', {
            cwd: rootPath,
            encoding: 'utf-8',
            timeout: 4000
          });
      ...
      case 'chanakya_graphify_query': {
        const graphify = GraphifyService.getInstance();
        ...
        if (args.action === 'summary') {
          ...
          if (!node) return `Could not find node for file: ${args.targetFile}`;
      ...
      case 'chanakya_terminal_exec': {
        ...
        try {
          const out = execSync(cmd, { cwd: rootPath, encoding: 'utf-8', timeout });
  ...
  public async readResource(uri: string): Promise<string> {
    if (uri === 'workspace://active_file') {
      ...
      if (!editor) return 'No active file open in editor.';
      ...
      const data = await GraphifyService.getInstance().generateGraphData(false);
