...
const planTracker_1 = require("./planTracker");
const executionGuard_1 = require("./executionGuard");
const toolSpillService_1 = require("./toolSpillService");
class AgentOrchestrator {
    static instance;
    logger = logger_1.Logger.getInstance();
    pendingUserOptionResolver = null;
    events = new events_1.EventEmitter();
    resolveUserOption(choice) {
...
`;
    ...
    async executeTool(name, args) {
        ...
        const guardIntervention = executionGuard_1.ExecutionGuardService.getInstance().evaluatePreExecution(name, args);
        ...
        try {
            ...
            switch (name) {
                ...
                case 'lsp_goto_definition': {
                    const results = await lspService_1.LspService.getInstance().goToDefinition(args.filePath, Number(args.line || 0), Number(args.character || 0));
                    rawResult = results.length === 0 ? 'No definitions found.' : JSON.stringify(results, null, 2);
                    break;
                }
                case 'lsp_goto_implementation': {
                    const results = await lspService_1.LspService.getInstance().goToImplementation(args.filePath, Number(args.line || 0), Number(args.character || 0));
                    rawResult = results.length === 0 ? 'No implementations found.' : JSON.stringify(results, null, 2);
                    break;
                }
                case 'lsp_find_references': {
                    const results = await lspService_1.LspService.getInstance().findReferences(args.filePath, Number(args.line || 0), Number(args.character || 0));
                    rawResult = results.length === 0 ? 'No references found.' : JSON.stringify(results, null, 2);
                    break;
                }
                case 'lsp_hover': {
                    const hover = await lspService_1.LspService.getInstance().hover(args.filePath, Number(args.line || 0), Number(args.character || 0));
                    rawResult = hover ? hover.contents.join('\n\n') : 'No hover information available.';
                    break;
                }
                case 'todo_write': {
                    const plan = planTracker_1.PlanTracker.getInstance().setPlan(args.title, args.tasks || []);
                    rawResult = `Plan initialized with ${plan.tasks.length} tasks. Current active step: "${plan.tasks[0]?.title || 'none'}".`;
                    break;
                }
                case 'plan_step': {
                    const updated = planTracker_1.PlanTracker.getInstance().updateTaskStatus(args.taskId, args.status);
                ...
                default:
                    if (name.startsWith('mcp_')) {
                        ...
                        rawResult = await mcpService_1.McpService.getInstance().callTool(serverName, originalToolName, args);
                    }
                    else {
                        throw new Error(`Unknown tool: ${name}`);
                    }
            }
            executionGuard_1.ExecutionGuardService.getInstance().recordToolResult(name, true);
            // 2. Tool output spill check
            const spillResult = await toolSpillService_1.ToolSpillService.getInstance().handleOutputSpill(name, rawResult);
            return spillResult.content;
        }
        catch (err) {
            executionGuard_1.ExecutionGuardService.getInstance().recordToolResult(name, false, err.message);
            this.logger.error(`[Agent] Tool ${name} failed`, err);
            return `Error executing ${name}: ${err.message}`;
        }
    }
    async runTerminalCommand(command, cwd) {
        return new Promise((resolve) => {
    ...
    async replaceInFile(filePath, targetContent, replacementContent) {
        const fullPath = this.resolvePath(filePath);
        try {
