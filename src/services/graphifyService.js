...
exports.GraphifyService = void 0;
...
class GraphifyService {
     ...
     */
    async generateGraphData(forceRefresh = false) {
        ...
        const serverListeningPorts = [];
        const backendServerEntrypoints = new Set();
        const getCommunity = (dirPath) => {
        ...
        const addEdge = (from, to, relation, type = 'import', label, title, dashes, arrows = 'to') => {
            if (!from || !to || from === to)
                return;
        ...
        const fileNameOnlyToNodeId = new Map();
        ...
        const resolveTargetNode = (sourceRelPath, importSpec) => {
            ...
            if (cleaned.startsWith('.')) {
                ...
                for (const e of exts)
                    candidatePaths.push(`${direct}${e}`);
                for (const e of ['.ts', '.tsx', '.js', '.jsx', '.java', '.php', '.py'])
                    candidatePaths.push(`${direct}/index${e}`);
            }
            ...
            try {
                ...
                if (['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.vue', '.svelte'].includes(ext)) {
                    ...
                    while ((match = classRegex.exec(content)) !== null) {
                        ...
                        if (fileNode && fileNode.symbols)
                            fileNode.symbols.push(symName);
                    ...
                    while ((match = angularTemplateRegex.exec(content)) !== null) {
                        ...
                        if (targetNodeId)
                            addEdge(fileNodeId, targetNodeId, 'template', 'import');
                    }
                    ...
                    while ((match = angularStyleRegex.exec(content)) !== null) {
                        ...
                        while ((sMatch = strRegex.exec(styleList)) !== null) {
                            ...
                            if (targetNodeId)
                                addEdge(fileNodeId, targetNodeId, 'styles', 'style');
                        }
                    }
                ...
                // ==========================================
                    ...
                    while ((match = djangoPathRegex.exec(content)) !== null) {
                        ...
                        if (viewTargetId)
                            addEdge(fileNodeId, viewTargetId, 'routes_to', 'import');
                    }
                ...
                // ==========================================
                    ...
                    while ((match = wpTemplateRegex.exec(content)) !== null) {
                        ...
                        if (targetId)
                            addEdge(fileNodeId, targetId, 'template_part', 'import');
                    }
                }
                // ==========================================
                // 6. Ruby & Rails
        ...
        for (const call of frontendApiCalls) {
            ...
            for (const route of backendRoutes) {
                if (call.fileNodeId === route.fileNodeId)
                    continue;
                const isExactMatch = call.normalizedRoute === route.normalizedRoute;
                const isPrefixMatch = call.normalizedRoute.startsWith(route.normalizedRoute) &&
            ...
            if (!matched && call.port) {
                for (const lp of serverListeningPorts) {
                    if (call.fileNodeId === lp.fileNodeId)
                        continue;
        ...
        const currentPath = [];
        const detectCycles = (u) => {
            visited.add(u);
            recStack.add(u);
            currentPath.push(u);
            const neighbors = adjacency.get(u) || [];
            for (const v of neighbors) {
                if (importCycles.length >= 5)
                    break; // cap cycles
                if (!visited.has(v)) {
                    detectCycles(v);
                }
        ...
        try {
            ...
            if (wsFolders && wsFolders.length > 0) {
                ...
                if (modifiedRelPaths.length > 0) {
                    const matchedNodes = nodes.filter((n) => modifiedRelPaths.some((p) => n.source_file.includes(p) || p.includes(n.source_file)));
     ...
     */
    async generateArchitectureMarkdown() {
...
`;
     ...
     */
    async exportArchitectureToFile() {
        const wsFolders = vscode.workspace.workspaceFolders;
        if (!wsFolders || wsFolders.length === 0) {
     ...
     */
    async calculateBlastRadius(targetNodeId, maxDepth = 3) {
        const data = await this.generateGraphData(false);
        ...
        for (const edge of data.edges) {
            if (!upstreamAdj.has(edge.to))
                upstreamAdj.set(edge.to, []);
     ...
     */
    async generateMermaidDiagram(maxEdges = 50) {
        const data = await this.generateGraphData(false);
        let mermaid = '```mermaid\nflowchart TD\n';
     ...
     */
    registerIncrementalWatcher(context) {
        ...
        context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(onFileEvent), vscode.workspace.onDidCreateFiles(onFileEvent), vscode.workspace.onDidDeleteFiles(onFileEvent), vscode.workspace.onDidRenameFiles(onFileEvent));
    }
}
exports.GraphifyService = GraphifyService;
//# sourceMappingURL=graphifyService.js.map