...
export class InlineEditCommand {
  private _provider: InlineEditProvider;
  ...
  constructor(provider: InlineEditProvider) {
    this._provider = provider;
  }

  public async handleInlineEdit() {
    ...
    await vscode.window.withProgress(
      ...
      async (_progress, token) => {
        try {
          ...
          const originalUri = vscode.Uri.parse(`${InlineEditProvider.scheme}:original-${timestamp}-${fileName}`);
          const modifiedUri = vscode.Uri.parse(`${InlineEditProvider.scheme}:modified-${timestamp}-${fileName}`);
          ...
          await vscode.commands.executeCommand(
            ...
            originalUri,
            modifiedUri,
            `Chanakya Inline Edit (Cmd+Shift+Enter to Accept)`
          );
