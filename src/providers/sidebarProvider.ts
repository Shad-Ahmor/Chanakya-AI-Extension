        ...
        if (config.longTask?.enabled) {
          ...
          if (complexity === TaskComplexity.LARGE || complexity === TaskComplexity.VERY_LARGE || complexity === TaskComplexity.EXTREME) {
            ...
            try {
              ...
              const requirementService = new RequirementExtractionService(LLMEngine.getInstance());
              const decomposer = new TaskDecomposer(ingestionService, requirementService, storageBaseDir, LLMEngine.getInstance());
