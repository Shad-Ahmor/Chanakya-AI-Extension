...
export interface ModelPricingProfile {
  ...
  textInputPricePerMillion: number; // in USD
  textTokensPerChar: number; // e.g. 0.35 (approx 1 token per 2.8 chars for code)
  imageTileTokens: (width: number, height: number) => number;
  tileDimensions: { width: number; height: number };
}

export const VISION_MODEL_PROFILES: Record<string, ModelPricingProfile> = {
  'claude-3-7-sonnet': {
...
export class VisionCostCalculator {
  /**
  ...
  public static calculateArbitrage(
    ...
    const profileKey = Object.keys(VISION_MODEL_PROFILES).find((k) =>
      modelId.toLowerCase().includes(k)
    ) || 'claude-3-7-sonnet';

    const profile = VISION_MODEL_PROFILES[profileKey];
