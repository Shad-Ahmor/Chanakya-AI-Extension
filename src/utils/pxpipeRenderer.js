...
exports.PxPipeRenderer = void 0;
const zlib = __importStar(require("zlib"));
const factsheetExtractor_1 = require("./factsheetExtractor");
class PxPipeRenderer {
     ...
     */
    static extractFactsheet(text) {
        return factsheetExtractor_1.FactsheetExtractor.extract(text).tokens;
    }
    /**
     * Render dense text into a high-density, crisp monospaced PNG Image.
     */
    static renderTextToPng(text, options = {}) {
        ...
        const savingsPercentage = Math.max(0, Math.round(((estimatedTextTokens - estimatedImageTokens) / (estimatedTextTokens || 1)) * 100));
        // Setup Dimensions
        ...
        const drawGlyph = (char, startX, startY, r, g, b) => {
        ...
        const drawString = (str, startX, startY, r, g, b) => {
            let curX = startX;
     ...
     */
    static encodeRgbaToPng(rgbaBuffer, width, height) {
    ...
    static getGlyphBitmap(code) {
        ...
        if (code >= 65 && code <= 90) {
            ...
            if (code === 65)
                return [0b011110, 0b100001, 0b100001, 0b111111, 0b100001, 0b100001, 0b100001, 0b100001, 0, 0]; // A
            if (code === 66)
                return [0b111110, 0b100001, 0b100001, 0b111110, 0b100001, 0b100001, 0b100001, 0b111110, 0, 0]; // B
            if (code === 67)
                return [0b011110, 0b100001, 0b100000, 0b100000, 0b100000, 0b100000, 0b100001, 0b011110, 0, 0]; // C
            if (code === 68)
                return [0b111100, 0b100010, 0b100001, 0b100001, 0b100001, 0b100001, 0b100010, 0b111100, 0, 0]; // D
            if (code === 69)
                return [0b111111, 0b100000, 0b100000, 0b111110, 0b100000, 0b100000, 0b100000, 0b111111, 0, 0]; // E
            if (code === 70)
                return [0b111111, 0b100000, 0b100000, 0b111110, 0b100000, 0b100000, 0b100000, 0b100000, 0, 0]; // F
            if (code === 71)
                return [0b011110, 0b100001, 0b100000, 0b100111, 0b100001, 0b100001, 0b100001, 0b011110, 0, 0]; // G
            if (code === 72)
                return [0b100001, 0b100001, 0b100001, 0b111111, 0b100001, 0b100001, 0b100001, 0b100001, 0, 0]; // H
            if (code === 73)
                return [0b011110, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0b011110, 0, 0]; // I
            if (code === 74)
                return [0b000011, 0b000001, 0b000001, 0b000001, 0b000001, 0b100001, 0b100001, 0b011110, 0, 0]; // J
            if (code === 75)
                return [0b100010, 0b100100, 0b101000, 0b110000, 0b101000, 0b100100, 0b100010, 0b100001, 0, 0]; // K
            if (code === 76)
                return [0b100000, 0b100000, 0b100000, 0b100000, 0b100000, 0b100000, 0b100000, 0b111111, 0, 0]; // L
            if (code === 77)
                return [0b100001, 0b110011, 0b101101, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0, 0]; // M
            if (code === 78)
                return [0b100001, 0b110001, 0b101001, 0b100101, 0b100011, 0b100001, 0b100001, 0b100001, 0, 0]; // N
            if (code === 79)
                return [0b011110, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0b011110, 0, 0]; // O
            if (code === 80)
                return [0b111110, 0b100001, 0b100001, 0b111110, 0b100000, 0b100000, 0b100000, 0b100000, 0, 0]; // P
            if (code === 81)
                return [0b011110, 0b100001, 0b100001, 0b100001, 0b100001, 0b100101, 0b100011, 0b011111, 0, 0]; // Q
            if (code === 82)
                return [0b111110, 0b100001, 0b100001, 0b111110, 0b101000, 0b100100, 0b100010, 0b100001, 0, 0]; // R
            if (code === 83)
                return [0b011110, 0b100001, 0b100000, 0b011110, 0b000001, 0b000001, 0b100001, 0b011110, 0, 0]; // S
            if (code === 84)
                return [0b111111, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0, 0]; // T
            if (code === 85)
                return [0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0b011110, 0, 0]; // U
            if (code === 86)
                return [0b100001, 0b100001, 0b100001, 0b100001, 0b010010, 0b010010, 0b001100, 0b001100, 0, 0]; // V
            if (code === 87)
                return [0b100001, 0b100001, 0b100001, 0b100001, 0b101101, 0b101101, 0b110011, 0b100001, 0, 0]; // W
            if (code === 88)
                return [0b100001, 0b100001, 0b010010, 0b001100, 0b010010, 0b100001, 0b100001, 0b100001, 0, 0]; // X
            if (code === 89)
                return [0b100001, 0b100001, 0b010010, 0b001100, 0b001100, 0b001100, 0b001100, 0b001100, 0, 0]; // Y
            if (code === 90)
                return [0b111111, 0b000010, 0b000100, 0b001000, 0b010000, 0b100000, 0b100000, 0b111111, 0, 0]; // Z
        }
        if (code >= 97 && code <= 122) {
            // Lowercase a-z
            if (code === 97)
                return [0, 0, 0b011110, 0b000001, 0b011111, 0b100001, 0b100001, 0b011111, 0, 0]; // a
            if (code === 98)
                return [0b100000, 0b100000, 0b111110, 0b100001, 0b100001, 0b100001, 0b100001, 0b111110, 0, 0]; // b
            if (code === 99)
                return [0, 0, 0b011110, 0b100001, 0b100000, 0b100000, 0b100001, 0b011110, 0, 0]; // c
            if (code === 100)
                return [0b000001, 0b000001, 0b011111, 0b100001, 0b100001, 0b100001, 0b100001, 0b011111, 0, 0]; // d
            if (code === 101)
                return [0, 0, 0b011110, 0b100001, 0b111111, 0b100000, 0b100001, 0b011110, 0, 0]; // e
            if (code === 102)
                return [0b001110, 0b010000, 0b111100, 0b010000, 0b010000, 0b010000, 0b010000, 0b010000, 0, 0]; // f
            if (code === 103)
                return [0, 0, 0b011111, 0b100001, 0b100001, 0b011111, 0b000001, 0b011110, 0b100000, 0]; // g
            if (code === 104)
                return [0b100000, 0b100000, 0b111110, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0, 0]; // h
            if (code === 105)
                return [0b001000, 0, 0b011000, 0b001000, 0b001000, 0b001000, 0b001000, 0b011100, 0, 0]; // i
            if (code === 106)
                return [0b000100, 0, 0b001100, 0b000100, 0b000100, 0b000100, 0b100100, 0b011000, 0, 0]; // j
            if (code === 107)
                return [0b100000, 0b100000, 0b100100, 0b101000, 0b110000, 0b101000, 0b100100, 0b100010, 0, 0]; // k
            if (code === 108)
                return [0b011000, 0b001000, 0b001000, 0b001000, 0b001000, 0b001000, 0b001000, 0b011100, 0, 0]; // l
            if (code === 109)
                return [0, 0, 0b110110, 0b101001, 0b101001, 0b101001, 0b101001, 0b101001, 0, 0]; // m
            if (code === 110)
                return [0, 0, 0b111110, 0b100001, 0b100001, 0b100001, 0b100001, 0b100001, 0, 0]; // n
            if (code === 111)
                return [0, 0, 0b011110, 0b100001, 0b100001, 0b100001, 0b100001, 0b011110, 0, 0]; // o
            if (code === 112)
                return [0, 0, 0b111110, 0b100001, 0b100001, 0b111110, 0b100000, 0b100000, 0b100000, 0]; // p
            if (code === 113)
                return [0, 0, 0b011111, 0b100001, 0b100001, 0b011111, 0b000001, 0b000001, 0b000001, 0]; // q
            if (code === 114)
                return [0, 0, 0b101110, 0b110001, 0b100000, 0b100000, 0b100000, 0b100000, 0, 0]; // r
            if (code === 115)
                return [0, 0, 0b011110, 0b100000, 0b011110, 0b000001, 0b100001, 0b011110, 0, 0]; // s
            if (code === 116)
                return [0b010000, 0b010000, 0b111100, 0b010000, 0b010000, 0b010000, 0b010001, 0b001110, 0, 0]; // t
            if (code === 117)
                return [0, 0, 0b100001, 0b100001, 0b100001, 0b100001, 0b100011, 0b011101, 0, 0]; // u
            if (code === 118)
                return [0, 0, 0b100001, 0b100001, 0b100001, 0b010010, 0b010010, 0b001100, 0, 0]; // v
            if (code === 119)
                return [0, 0, 0b100001, 0b100001, 0b101101, 0b101101, 0b110011, 0b100001, 0, 0]; // w
            if (code === 120)
                return [0, 0, 0b100001, 0b010010, 0b001100, 0b010010, 0b100001, 0b100001, 0, 0]; // x
            if (code === 121)
                return [0, 0, 0b100001, 0b100001, 0b100001, 0b011111, 0b000001, 0b011110, 0, 0]; // y
            if (code === 122)
                return [0, 0, 0b111111, 0b000010, 0b000100, 0b001000, 0b010000, 0b111111, 0, 0]; // z
        }
        // Punctuation & Symbols
        const punct = {
...
exports.PxPipeRenderer = PxPipeRenderer;
//# sourceMappingURL=pxpipeRenderer.js.map